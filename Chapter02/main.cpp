...
const char* keys =
{
	"{help h usage ? | | print this message}" // -- or - plus help/h/usage/? to display help info
...
{
	CommandLineParser parser(argc, argv, keys); // keys = string describing acceptable command line parameters
    parser.about("Chapter 2. v1.0.0"); // will be shown when printMessage is called, right before arguments table.
    //If requires help show
    if (parser.has("help"))
	{
	    parser.printMessage(); // This method will print standard help message containing the about message and arguments description.
	    return 0;
	}
	...
	// Returns false if error occurred while accessing the parameters (bad conversion, missing arguments, etc.). 
	if (!parser.check())
	{
	    parser.printErrors();
	    return 0;
	...
	switch(sample)
	{
		...
		// CV types corresponds to C data types
